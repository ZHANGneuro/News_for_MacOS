...
- (void)commonInit {
    ...
    NSUserDefaults *wangyi_check = [NSUserDefaults standardUserDefaults];
    NSInteger wangyiStatus = [wangyi_check integerForKey:@"wangyi"];
    if (wangyiStatus==1){
       _on = YES;
    }else{
       _on = NO;
...
- (void)mouseDown:(NSEvent *)theEvent{
    if ([public_info Button_wangyi_enabled]) {
...
- (void)setOn:(BOOL)on animated:(BOOL)animated {
    ...
    if (on) {
        [public_info setButton_wangyi_enabled:NO];
        NSUserDefaults *wangyi_check = [NSUserDefaults standardUserDefaults];
        NSInteger wangyiStatus = [wangyi_check integerForKey:@"wangyi"];
        if (wangyiStatus==0){
            [self addOnAnimation];
